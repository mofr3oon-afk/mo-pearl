...
function guestFromDescription(text){
  ...
  for(const line of block.split(/\n+/)){
    ...
    for(const token of safe.match(/[A-Za-z]+/g)||[])if(!banned.test(token)&&(token.length>=3||/^(Mr|HB)$/i.test(token)))words.push(token);
  }
...
export function parseInvoiceOcr(texts,descriptionText=''){
  ...
  return {invoiceNo:internal,invoiceDate:issuance.replaceAll('/','-'),companyTax,guest:guestFromDescription(descriptionText)||guestFrom(texts.find(t=>/Accommodation/i.test(t))||all),nights,roomRate:accommodation&&nights?Math.round((accommodation/nights)*100)/100:'',accommodation,restaurant,laundry,total,verified:Boolean(total&&combo&&Math.abs(sum-total)<=1)};
}
