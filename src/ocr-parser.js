...
export function parseInvoiceOcr(texts,descriptionText=''){
  ...
  return {invoiceNo:internal,invoiceDate:issuance.replaceAll('/','-'),guest:guestFromDescription(descriptionText)||guestFrom(texts.find(t=>/Accommodation/i.test(t))||all),accommodation,restaurant,laundry,total,verified:Boolean(total&&combo&&Math.abs(sum-total)<=1)};
}
