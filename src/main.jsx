...
import './styles.css';
import { parseInvoiceOcr } from './ocr-parser.js';

const seedCompanies = [{ id: crypto.randomUUID(), code: 'EXPRO', name: 'EXPRO EGYPT LLC', tax: '204-829-542', invoiceTax: '205938604', po: '4300228298' }];
...
function App(){
  const [companies,setCompanies]=useState(()=>{const saved=JSON.parse(localStorage.getItem('invoice-companies')||'null');return (saved||seedCompanies).map(c=>({...c,invoiceTax:c.invoiceTax||(c.code==='EXPRO'?'205938604':String(c.tax||'').replace(/\D/g,''))}));});
  ...
  async function readImage(){if(!file)return;setReading(true);setProgress(3);try{
    ...
    await worker.setParameters({tessedit_pageseg_mode:'6',preserve_interword_spaces:'1'});const description=(await worker.recognize(cropCanvas(canvas,.03,.50,.40,.87))).data.text;await worker.terminate();
    const parsed=parseInvoiceOcr(texts,description);const matched=companies.find(c=>digitDistance(c.invoiceTax||c.tax,parsed.companyTax)<=1);if(matched)setCompanyId(matched.id);
    setInvoice(p=>{const next={...p,...Object.fromEntries(Object.entries(parsed).filter(([k,v])=>k!=='verified'&&v!==null))};next.amountWords=amountInWords(parsed.total);return next});
