...
function App(){
  ...
  const setInv=(k,v)=>setInvoice(p=>{const next={...p,[k]:v};if(k==='checkIn')next.invoiceDate=v;if(k==='nights'||k==='roomRate'){const n=money(next.nights),rate=money(next.roomRate);if(n&&rate)next.accommodation=Math.round(n*rate*100)/100;}return next;});
