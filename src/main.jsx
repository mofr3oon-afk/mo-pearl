...
const emptyInvoice = { invoiceNo: '', invoiceDate: '', roomNo: '', guest: '', checkIn: '', checkOut: '', nights: 1, roomRate: '', accommodation: '', restaurant: '', amountWords: '' };
...
function parseOcr(text) {
  ...
  const guest = clean.match(/Accommodation\s*(?:Services)?[\s/:-]+([A-Za-z][A-Za-z ]{4,40})/i)?.[1]?.trim() || '';
  const accommodationBlock = clean.match(/Accommodation[\s\S]{0,700}?(?=Food\s+and\s+Drink|$)/i)?.[0] || '';
  const restaurantBlock = clean.match(/Food\s+and\s+Drink[\s\S]{0,700}?(?=Total\s+Sales|$)/i)?.[0] || '';
  const blockAmounts = (block) => [...block.matchAll(/\b(\d{2,5}(?:,\d{3})*(?:\.\d{2,4})?)\b/g)].map(x => money(x[1])).filter(n => n > 20 && n < 100000);
  const accommodation = Math.max(0, ...blockAmounts(accommodationBlock));
  const restaurant = Math.max(0, ...blockAmounts(restaurantBlock));
  const total = [...clean.matchAll(/Total\s*Amount\s*\(EGP\)\s*([\d,.]+)/gi)].at(-1)?.[1] || '';
  return { invoiceNo: internal, invoiceDate: issuance.replaceAll('/', '-'), guest, accommodation: accommodation || '', restaurant: restaurant || '', total: money(total) || '' };
}
...
async function createExcel(company, inv) {
  ...
  const rows=[['Accommodation',inv.checkIn,inv.checkOut,inv.nights,'','',inv.nights,money(inv.roomRate)||accommodation,accommodation],['Restaurant','','','','','','','',money(inv.restaurant)]];
  ...
  ['H14','I14','I15','I23','I25'].forEach(a=>ws.getCell(a).numFmt='#,##0.00');
...
function App(){
  ...
  async function readImage(){if(!file)return;setReading(true);setProgress(1);try{const {createWorker}=await import('tesseract.js');const worker=await createWorker(['eng','ara'],1,{logger:m=>m.status==='recognizing text'&&setProgress(Math.round(m.progress*100))});const {data}=await worker.recognize(file);await worker.terminate();const parsed=parseOcr(data.text);setInvoice(p=>{const next={...p,...Object.fromEntries(Object.entries(parsed).filter(([k,v])=>k!=='total'&&v!==''&&v!==0))};next.amountWords=next.amountWords||amountInWords(parsed.total||money(next.accommodation)+money(next.restaurant));return next});setNotice('تمت القراءة. راجع البيانات قبل تنزيل الشيت.');}catch(e){setNotice('تعذرت قراءة الصورة تلقائيًا. اكتب البيانات يدويًا ثم نزّل الشيت.');}finally{setReading(false);setProgress(0);}}
  async function download(){if(!company){setNotice('أضف شركة أولًا.');return;}const finalInvoice={...invoice,amountWords:invoice.amountWords||amountInWords(money(invoice.accommodation)+money(invoice.restaurant))};setInvoice(finalInvoice);await createExcel(company,finalInvoice);setNotice(`تم إنشاء ${company.code} - ${dateForFile()}.xlsx`);}
  return <div className="app-shell">
    ...
    <main>{tab==='create'?<>
      ...
      <div className="workspace"><section className="step-card upload-panel"><div className="section-title"><span className="step-no">2</span><div><h2>ارفع الفاتورة</h2><p>صورة واضحة أو ملف صورة ممسوح</p></div></div><label className={`dropzone ${preview?'has-file':''}`}><input type="file" accept="image/*" onChange={e=>pickFile(e.target.files[0])}/>{preview?<img src={preview} alt="معاينة الفاتورة"/>:<><UploadCloud/><strong>اضغط لاختيار الصورة</strong><span>JPG أو PNG</span></>}</label><button className="primary wide" disabled={!file||reading} onClick={readImage}>{reading?<><span className="spinner"/>جاري القراءة {progress}%</>:<><ScanText/>قراءة بيانات الفاتورة</>}</button><div className="privacy"><Check/>الصورة لا يتم حفظها في قاعدة بيانات</div></section>
