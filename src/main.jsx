...
function parseOcr(text) {
  ...
  const servicesTotal=Math.round((accommodation+restaurant+laundry)*100)/100;
  return { invoiceNo: internal, invoiceDate: issuance.replaceAll('/', '-'), guest, accommodation: accommodation || '', restaurant: restaurant || '', laundry: laundry || '', total: servicesTotal || parsedNumber(total) || '' };
}
...
function cropCanvas(source, top=.28, bottom=.92){const canvas=document.createElement('canvas');canvas.width=source.width;canvas.height=Math.round(source.height*(bottom-top));canvas.getContext('2d').drawImage(source,0,Math.round(source.height*top),source.width,canvas.height,0,0,canvas.width,canvas.height);return canvas;}
...
function App(){
  ...
  async function readImage(){if(!file)return;setReading(true);setProgress(3);try{
    ...
    await worker.setParameters({tessedit_pageseg_mode:'6',preserve_interword_spaces:'1'});const full=await worker.recognize(canvas);
    await worker.setParameters({tessedit_pageseg_mode:'11',preserve_interword_spaces:'1'});const services=await worker.recognize(cropCanvas(canvas));await worker.terminate();
    const parsed=parseOcr(`${full.data.text}\n${services.data.text}`);
