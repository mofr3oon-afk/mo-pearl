...
function App(){
  ...
  useEffect(()=>{let active=true;readDraftMedia().then(saved=>{if(!active||!saved)return;
    ...
    if(saved.folio){setFolioFile(saved.folio);setFolioPreview(URL.createObjectURL(saved.folio));}
  ...
  useEffect(()=>{if(!mediaRestored)return;writeDraftMedia({invoice:file,folio:folioFile,po:poFile,pages:extraPages.map(p=>p.file)}).catch(()=>setDraftStatus('تم حفظ البيانات، لكن صور المسودة لم تُحفظ (راجع مساحة الجهاز)'));},[file,folioFile,poFile,extraPages,mediaRestored]);
  ...
  function newInvoice(){setRestoredId('');setInvoice({...emptyInvoice});setFile(null);setFolioFile(null);setPoFile(null);setPoText('');setExtraPages(old=>{old.forEach(p=>URL.revokeObjectURL(p.url));return [];});setInvoicePo(company?.po||'');setPreview('');setFolioPreview('');setPoPreview('');setFolioChecks(null);setNotice('تم فتح فاتورة جديدة.');}
